...
class PlotDenyCommand(private val plugin: PlotPlugin) : CommandBase(
    ...
    private val playerArg = withRequiredArg(
        "player",
        "souza.plot.command.deny.player.desc",
        ArgTypes.STRING
    )
    ...
    override fun executeSync(context: CommandContext) {
        ...
        if (targetName.isNullOrBlank()) {
            context.sendMessage(Message.raw("Usage: /plot deny <player>"))
            return
        }
        ...
        // Deny player
        if (plugin.plotManager.denyPlayer(plotId, targetPlayer.uuid)) {
            context.sendMessage(Message.raw(plugin.config.messages.format(
                plugin.config.messages.playerDenied, targetPlayer.username
            )))
        } else {
            context.sendMessage(Message.raw(plugin.config.messages.format(
                plugin.config.messages.playerAlreadyDenied, targetPlayer.username
            )))
        }
    }
}
