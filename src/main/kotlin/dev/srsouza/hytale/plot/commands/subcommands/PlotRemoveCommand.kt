...
class PlotRemoveCommand(private val plugin: PlotPlugin) : CommandBase(
    ...
    private val playerArg = withRequiredArg(
        "player",
        "souza.plot.command.remove.player.desc",
        ArgTypes.STRING
    )
    ...
    override fun executeSync(context: CommandContext) {
        ...
        if (targetName.isNullOrBlank()) {
            context.sendMessage(Message.raw("Usage: /plot remove <player>"))
            return
        }
        ...
        // Remove member
        if (plugin.plotManager.removeMember(plotId, targetPlayer.uuid)) {
            context.sendMessage(Message.raw(plugin.config.messages.format(
                plugin.config.messages.memberRemoved, targetPlayer.username
            )))
        } else {
            context.sendMessage(Message.raw(plugin.config.messages.format(
                plugin.config.messages.memberNotFound, targetPlayer.username
            )))
        }
    }
}
